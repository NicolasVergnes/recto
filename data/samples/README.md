...
Fixtures **synthétiques**, générées le 25/09/2026 par script (Python `sqlite3` + `zipfile`), conformes au schéma legacy 11 d'Anki tel que documenté par le wiki AnkiDroid (« Database Structure »). Elles n'ont pas été produites par Anki (le paquet `image-occlusion.apkg` ci-dessus l'a été ; `tests/interop/` vérifie aussi les paquets exportés par Recto avec le moteur d'Anki). Pour un autre test d'interopérabilité, exporter un paquet depuis Anki avec l'option « Prise en charge des anciennes versions d'Anki » et l'ajouter ici sous un autre nom (les paquets AnkiWeb sont sous licence de leurs auteurs : ne pas les commiter).
