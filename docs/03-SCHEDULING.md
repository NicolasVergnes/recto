...
- V1 : optimisation dans le navigateur avec `fsrs-browser` (WASM, `Fsrs.computeParameters(ratings, delta_ts, lengths, …)`) dans un Web Worker module à usage unique, proposée quand le paquet cumule ≥ 1 000 révisions utilisables ; avant d'accepter, l'utilisateur compare anciens et nouveaux paramètres (erreur du modèle, pénalité des erreurs, réussite prédite vs réelle, intervalles d'exemple, 21 valeurs). Le module WASM n'est chargé que par ce worker, jamais dans le bundle initial.

*Mise en œuvre (M6).*

- **Données** (`src/lib/db/optimize.ts`) : les révisions des cartes **propres** au paquet (chaque sous-paquet a ses réglages), réponses Leitner comprises (1/3/4 sont des notes FSRS valides). Le seuil de 1 000 porte sur les révisions **utilisables** (celles que garde le jeu d'entraînement ci-dessous) ; les autres (carte importée sans historique, réponses antérieures à une remise à zéro, carte jamais revue un autre jour) sont comptées à part et expliquées.
...
- **Worker** (`optimizer.worker.ts`, à usage unique, terminé après la réponse, à l'expiration du délai de garde de 2 min ou quand on quitte les réglages) : `init({ module_or_path })` puis `computeParameters(…, null, true, null, nombre de pas de réapprentissage)` en **mono-thread** (`initThreadPool` jamais appelé : pas besoin d'isolation cross-origin). Le nombre de pas de réapprentissage borne w17/w18 comme le fait ts-fsrs à la planification. Mémoire WASM partagée non disponible → « indisponible dans ce navigateur » ; tout autre échec (chargement du `.wasm`, calcul) → « échec », qu'on peut retenter.
- **Évaluation** (TypeScript, sur le même historique) : `next_state`/`forgetting_curve` de ts-fsrs, qui reproduisent les états de fsrs-rs, avec les réglages du paquet (donc le même plafonnement de w17/w18 qu'à la planification). Pour chaque réponse après un écart > 0 : perte logarithmique, RMSE sur 20 classes de probabilité, réussite prédite moyenne vs réelle ; plus les intervalles d'une nouvelle carte toujours notée « Bien » (réglages du paquet). Paramètres arrondis à 4 décimales et validés (21 nombres finis).
- **Décision** : si fsrs-rs renvoie exactement les paramètres par défaut (son signal de données trop rares), « Pas assez de données », rien à appliquer. Sinon, « Appliquer » n'est proposé que si la perte logarithmique baisse sur les révisions passées (comparaison sur les données d'entraînement : le texte parle de révisions passées, pas de mémoire en général) ; sinon « Vos paramètres actuels conviennent déjà ». « Appliquer » n'écrit que `settings.fsrs.params` et termine la séance en cours ; « Revenir aux paramètres par défaut » remet `null`. Les échéances existantes ne sont **pas recalculées** : les nouveaux paramètres s'appliquent à chaque carte lors de sa prochaine réponse.
