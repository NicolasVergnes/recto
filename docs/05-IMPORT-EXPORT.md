...
- **Occlusion d'image** (V1) : un modèle `type = 1` dont un template contient `image-occlusion` (type natif « Image Occlusion » d'Anki ≥ 23.10 ; noms de champs traduits, d'où la détection par le template) → `image_occlusion` : champs dans l'ordre [Occlusion, Image, En-tête, Back Extra, Comments] → [Image, masques, En-tête, Back Extra (+ Comments)]. Chaque forme `{{cN::image-occlusion:rect:left=…:top=…:width=…:height=…[:oi=1]}}` devient un masque du groupe N (`oi=1` sur une forme → mode « tout cacher ») ; ellipses et polygones deviennent leur rectangle englobant ; les formes tournées (`angle`) sont ignorées — la rotation se fait en pixels et le rapport largeur/hauteur de l'image est inconnu ici, aucun rectangle normalisé n'est sûr de couvrir la zone (P1) —, comme les formes texte et les coordonnées en pixels ; ces conversions et omissions sont comptées dans le rapport (pour les notes créées ou mises à jour seulement). Si Comments ne contient que des lignes `n : réponse` (écrites par l'export de Recto, §4), ce sont les réponses des masques : chacune revient au premier masque de son groupe ; sinon Comments rejoint l'extra. Une note sans masque lisible est une erreur « occlusion d'image sans masque lisible ». Fixture : `data/samples/image-occlusion.apkg` (exportée par Anki).
...
- **Cartes** : créer une carte Recto par carte Anki (`ord` conservé ; pour `basic_reverse`, `ord` 0/1). État :
  ...
  - Sans historique importé : `type 0` → `New` ; `type 2` → `Review` avec pour échéance le début du jour d'étude situé `due` jours calendaires après la date locale de `crt` (jour 0 d'Anki ; des jours et non `due × 86 400 s`, qui décalerait d'un jour au changement d'heure près du début de journée), `scheduledDays = ivl`, `stability = ivl` (approximation), `difficulty = clamp(11 − factor/1000 × 2, 1, 10)` ; `type 1/3` → `Learning` due maintenant. Pour un paquet cible Leitner, `box` selon la table de 03-SCHEDULING §4.
...
- `data/samples/image-occlusion.apkg` : paquet **exporté par le moteur d'Anki 26.9** (type natif « Image Occlusion », rectangle, ellipse, polygone, carte suspendue, média) ; `tests/unit/import/apkg-occlusion.test.ts`. Aller-retour export/import Recto (réponses des masques comprises) : `tests/unit/export-apkg.test.ts` ; contrôle manuel dans Anki : `tests/interop/build-occlusion-sample.test.ts` + `check_apkg.py`.
...
- **Types de notes** : quatre modèles à identifiants et `mod` fixes, aux noms distinctifs (une réimportation dans Anki les réutilise, sans copie « + », et n'écrase pas un type modifié dans Anki) : « Recto · Basique » (Recto, Verso, Extra), « Recto · Basique et inversée » (2 modèles de carte, le 2ᵉ demande `{{Verso}}`), « Recto · Texte à trous » (Texte, Extra, `{{cloze:Texte}}`), « Recto · Occlusion d'image » : copie du type natif « Image Occlusion » d'Anki ≥ 23.10 (`type = 1`, `originalStockKind = 6`, champs Occlusion, Image, Header, Back Extra, Comments avec leurs `tag` 0–4, modèle et CSS d'Anki 26.9) pour que le script d'Anki dessine les masques et que son éditeur de masques ouvre ces notes. Champs d'une note d'occlusion : `{{cN::image-occlusion:rect:left=…:top=…:width=…:height=…[:oi=1]}}` par masque (joints par `<br>`, nombres sans zéros inutiles, `oi=1` en mode « tout cacher »), l'image, l'en-tête, l'extra, et dans Comments (non affiché par Anki) les réponses des masques, une ligne par groupe (`2 : Lyon`, HTML échappé), qu'Anki ne sait pas représenter ; réimportées dans Recto (nouvelle note ou mise à jour d'une note modifiée dans Anki), elles redeviennent des réponses, sur le premier masque de chaque groupe. Vérifié avec le moteur d'Anki 26.9.3 (`tests/interop/`).
...
- **Cartes** : `id` = `createdAt` (même règle de collision), `ord` inchangé, `flags` = drapeau, `reps`/`lapses` copiés ; `suspended` **ou `retired`** → `queue = −1` (Anki n'a pas d'équivalent à « retirée » : signalé dans le rapport) ; f(D) = clamp(round((11 − D) × 500), 1300, 5000), inverse de la conversion à l'import, pour FSRS, 2500 pour Memory Box.
  ...
  `cards.data` : `{"s", "d", "dr", "lrt"}` (état mémoire FSRS, paquets FSRS seulement) ou `{"lrt"}` (date de dernière révision, que « Vérifier la base de données » ajouterait sinon). `col.crt` : sa date locale est le « jour 0 » d'Anki, ici le jour d'étude de l'échéance de révision la plus ancienne (au plus aujourd'hui), donc toutes les échéances en jours sont ≥ 0. Anki lit cette date avec `conf.creationOffset` : sans lui, il la lirait avec le décalage UTC du moment de l'import, et une `crt` en heure d'hiver importée en heure d'été décalait toutes les échéances d'un jour pour un début de journée à 22 h ou 23 h (vérifié avec Anki 26.9.3, corrigé par `creationOffset`). `crt` est placé à mi-chemin entre le début du jour d'étude et minuit, pour que sa date locale soit sans ambiguïté le jour d'étude voulu. L'import de Recto ajoute des jours calendaires à la date locale de `crt` (M6) : l'aller-retour garde chaque échéance dans son jour d'étude pour tout début de journée, 23 h compris (test « keeps review dues on their study day across daylight saving », heures 0, 4, 12, 22 et 23). Anki recale les échéances sur son propre « aujourd'hui » à l'import (vérifié).
