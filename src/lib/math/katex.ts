 ...
 * between formulas). An invalid formula keeps its source, marked `math-error`, with the message
 * as a `title` (mouse) and as visually hidden text (screen readers; no hover on a phone).
 * `render()` is not used: it empties the element before parsing and throws in quirks mode.
 */
export function renderMath(root: ParentNode, invalidMessage: string): void {
