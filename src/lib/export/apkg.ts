...
/**
 * Anki's own « Image Occlusion » note type (Anki ≥ 23.10, `originalStockKind` 6): its template
 * script draws the masks, and the field tags let Anki's mask editor open the notes. Copied from
 * Anki 26.9; field order [Occlusion, Image, Header, Back Extra, Comments] is what Recto imports.
 */
const IO_FRONT = `{{#Header}}<div>{{Header}}</div>{{/Header}}
...
`
const IO_FIELDS = ['Occlusion', 'Image', 'Header', 'Back Extra', 'Comments']
...
function model(type: ModelType) {
  ...
  if (type === 'image_occlusion')
    return {
      ...common,
      name: 'Recto · Occlusion d’image',
      type: 1,
      css: IO_CSS,
      originalStockKind: 6,
      flds: IO_FIELDS.map((name, ord) => ({
        ...field(name, ord),
        tag: ord,
        preventDeletion: ord < 4,
      })),
      tmpls: [template('Image Occlusion', 0, IO_FRONT, IO_BACK)],
      req: [[0, 'any', [0, 1, 2]]],
    }
  if (type === 'cloze')
    return {
...
/**
 * A note's fields in the order of its Anki note type. Occlusion notes: [Occlusion, Image, Header,
 * Back Extra, Comments]; mask answers have no Anki equivalent and are listed in Comments, which
 * Anki does not display (« 2 : Lyon »).
 */
export function ankiFields(note: Pick<Note, 'modelType' | 'fields'>): string[] {
...
export async function buildApkg(
  ...
  const sorted = await Promise.all(
    notes.map(async (note) => {
      const fields = ankiFields(note)
      const sfld = sortField(fields[0] ?? '')
      return { note, fields, sfld, csum: await fieldChecksum(sfld) }
    }),
  )
  ...
  try {
    ...
    for (const { note, fields, sfld, csum } of sorted) {
    ...
    const models = Object.fromEntries(MODEL_TYPES.map((t) => [ANKI_MODEL_IDS[t], model(t)]))
